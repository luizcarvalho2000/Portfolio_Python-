...
from Business.PreferenciasBusiness import *
...
class FrmLstbxPreferencias(tk.Frame):
    def __init__(self, master, *args, **kwargs):
        ...
        self.lstbxPreferencias.place(x=5, y=5, width=200, height=270)

        barraDeRolagem = Scrollbar(self, orient="vertical", command=self.lstbxPreferencias.yview)
        barraDeRolagem.place(x=285, y=1, width=15, height=270)
...
class FrmDtgvwPreferencias(tk.Frame):
    def __init__(self, master, Preferencias, *args, **kwargs):
        ...
        for i in range(len(self.colunas)):
            ...
            self.dtgvwPreferencias.column(self.colunas[i], width=self.tamanhaColunas[i], anchor=self.ancoramentoDasColunas[i])
